...
wordArray = [word.lower() for word in wordArray if word.lower() not in new_stopwords]
...
wordArray = [WordNetLemmatizer().lemmatize(t[0], get_wordnet_pos(t[1])) for t in pos_tags]
...
def document_features(document):
    features = {}
    documentWords = set(document) # want to get all of the unique words in a given message
    for word in wordFeatures:
        features['contains({})'.format(word)] = (word in documentWords)
    return features

featureSets = [(document_features(text[i]),objective[i]) for i in range(0,len(text))]
#shuffle(featureSets)
    ...
    for outer_idx, obj_review_1 in enumerate(obj_list):
        ...
        for inner_idx, obj_review_2 in enumerate(obj_list):
            ...
            if outer_idx == inner_idx:
                ...
                cos_sim = 1 - spatial.distance.cosine(obj_review_1, obj_review_2)
        ...
        temp_average_subj = sum(cos_sim_tracker_subj) / len(cos_sim_tracker_subj)
        
        #store all information in temp tumple, then add temp_tuple to outer_obj list
        temp_tuple = (f'Objective Review: {outer_idx}', temp_average_obj, temp_average_subj)
    ...
    for outer_idx, subj_review_1 in enumerate(sub_list):
        ...
        for inner_idx, subj_review_2 in enumerate(sub_list):
            ...
            if outer_idx == inner_idx:
                ...
                cos_sim = 1 - spatial.distance.cosine(subj_review_1, subj_review_2)
                cos_sim_tracker_subj.append(cos_sim)
                #print(f'cos_sim = {cos_sim}')
        ...
        #now calculate average cos_sim for subj_review_1 and each other subjective review
        temp_average_obj = sum(cos_sim_tracker_obj) / len(cos_sim_tracker_obj)
        temp_tuple = (f'Subjective Review: {outer_idx}' , temp_average_obj, temp_average_subj)
...
obj_binary, subj_binary = binary_checker(obj_sentences_list_tokenized, subj_sentences_list_tokenized)
